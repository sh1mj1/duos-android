...
/** * RecyclerView item decoration that draws sticky headers over the content. The only thing that has to be done is to * implement the [StickyHeader] interface in the [RecyclerView.ViewHolder]s that have to become sticky. * * Only one sticky header at a time is supported for now. * * **Note** When removal of the [StickyHeaderDecoration] is needed, **always** use [StickyHeaderDecoration.release] * to clear touch and scroll listeners needed for the sticky creation to work * * Although not necessary usage of DiverseRecyclerAdapter simplifies the creation of sticky headers. */
class StickyHeaderDecoration : RecyclerView.ItemDecoration() {
    private val stickyHeadersMap: MutableMap<Any, RecyclerView.ViewHolder?> =
        linkedMapOf() // Used for optimisation, creating new instances during draw calls is dangerous
    ...
    /** * Call this when you need to update the sticky headers without notifying the [RecyclerView.Adapter] * for changes */
    ...
    override fun onDrawOver(canvas: Canvas, recyclerView: RecyclerView, state: RecyclerView.State) {
        ...
        stickyOffsets.clear() // Reversed order to try to catch views that can disappear before we reach them
        ...
        ((recyclerView.childCount - 1) downTo 0).asSequence()
            ...
            .forEach { viewToViewHolderPair ->
                ...
                val adapterPosition =
                    (stickyHeaderViewHolder as RecyclerView.ViewHolder).adapterPosition


                if (viewTop < STICKY_THRESHOLD || (scrollDeltaY > viewTop && recyclerView.canScrollVertically(
                        1
                    ))
                ) {
                ...
                currentStickyId =
                    if (stickiesStack.isNotEmpty()) stickiesStack.peek() else null
                if (adapterPosition != -1) {
        ...
        currentStickyId?.let { currentStickyId ->
            ...
            val candidateTop = stickyOffsets.asSequence()
                .lastOrNull { it.value in 0..currentStickyHeight }
            if (candidateTop != null) {
                val currentMargin =
                    (currentStickyHeight - candidateTop.value).toFloat()
    ...
    private fun registerListeners() {
        ...
        adapter?.apply {
            if (adapterObserver == null) {
                registerAdapterDataObserver(AdapterObserver().also { adapterObserver = it })
            }
        }
    }
    ...
    private fun createOrUpdateStickyHeader(
        ...
        val stickyItemViewType =
            (stickyViewHolder as RecyclerView.ViewHolder).itemViewType
        this.stickyItemViewType =
            stickyItemViewType
        val adapterPosition =
            (stickyViewHolder as RecyclerView.ViewHolder).adapterPosition
        stickyHeadersMap.getOrPut(stickyId) {
            val adapter = recyclerView.adapter!!
            val newStickyViewHolder = adapter.onCreateViewHolder(
                recyclerView,
                stickyItemViewType
            )
            ...
            val viewWidth = ViewGroup.getChildMeasureSpec(
                ...
                stickyView.layoutParams?.width ?: 0
            )
            val viewHeight = ViewGroup.getChildMeasureSpec(
                heightSpec,
                recyclerView.paddingTop + recyclerView.paddingBottom,
                stickyView.layoutParams?.height ?: 0
            )
            val newStickyItemView =
                newStickyViewHolder.itemView
    ...
    private inner class ItemTouchListener : RecyclerView.SimpleOnItemTouchListener() {
        override fun onInterceptTouchEvent(
            ...
            if (event.action == MotionEvent.ACTION_MOVE || (event.action == MotionEvent.ACTION_UP && recyclerView.scrollState == RecyclerView.SCROLL_STATE_DRAGGING)) {
                return false
            }
            currentStickyId?.let {
    ...
    private inner class AdapterObserver : RecyclerView.AdapterDataObserver() {
        override fun onItemRangeChanged(positionStart: Int, itemCount: Int) {
            val changedRange =
                positionStart..(positionStart + itemCount)
        ...
        override fun onItemRangeRemoved(positionStart: Int, itemCount: Int) {
            stickyHeadersMap.forEach { entry ->
                val adapterPosition = adapterPositionsMap[entry.key]
                    ?: return@forEach
